...
export default function CampaignMap() {
  ...
  // Aggregate completed days per category
  ...
  for (const [category, dates] of catMap) {
    frontLines.push({ category, totalDays: dates.size })
  }
  ...
  // Include categories with habits but no completions
  for (const h of habits) {
    if (!frontLines.some((f) => f.category === h.category)) {
      frontLines.push({ category: h.category, totalDays: 0 })
    }
  }
